...
import { compare, regTest } from '../lib';
    ...
    switch (action.type) {
      case CREATE_D_ATRIBUTES:
        ...
        for (let parent in action.payload) {
          _state = { ...state, [parent]: Object.assign({}, state[parent], action.payload[parent])};
        }
        // console.log('CREATE_D_ATRIBUTES', _state);
        return _state;
      case CHANGE_INPUT_VALUE:
        ...
        for (let parent in action.payload) {
          for (let field in action.payload[parent]) {
            ...
            _change = { ...state, [parent]: { ...state[parent], [field]: {
              ...state[parent][field], value: action.payload[parent][field].value
            }}};
          }
        }
        return _change;      
